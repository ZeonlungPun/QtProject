...
//使用 QTableView 的 clicked 或 activated 信號來偵測使用者點擊的資料行，
 //然後從資料庫中取出對應的圖片 BLOB，轉換為 QPixmap 並顯示在 QLabel（如 ui->portraitWindow）中
    ...
    connect(this, &ChildWindow::startDetect, Detect_work, &Detect::working);

    //only start the sub-process when it recieve the signals
    detect_task->start();

    // post-process the detect/recognize results
    connect(Detect_work, &Detect::detectFinished, this, &ChildWindow::onDetectFinished);


}

void ChildWindow::onDetectFinished(const cv::Mat& coordinates,bool faceDetected)
{
    QString currentDateTime = QDateTime::currentDateTime().toString("yyyy-MM-dd hh:mm:ss");
    if (faceDetected)
    {
        cv::Mat sourceface= this->Portrait_img(cv::Rect(coordinates.at<float>(0, 0),coordinates.at<float>(0, 1),coordinates.at<float>(0, 2),coordinates.at<float>(0, 3)));
...
void ChildWindow::on_insertBtn_clicked()
{
    ...
    this->Portrait_img= cv::imread(selectedPortraitPath.toStdString());
    if (this->Portrait_img.empty()) {
        ui->LoggingWindow->append(QString("[ERROR %1] %2").arg(currentDateTime).arg("圖像讀取失敗！"));
        return;
    }

    // Set input size before inference
    detector->setInputSize(this->Portrait_img.size());
    emit startDetect(this->Portrait_img);
